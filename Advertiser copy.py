        ...
        if update.message is not None:
            if update.message.chat is not None:
                if update.message.chat.type == 'private':
                    connection = DbConnect()
                    cursor = connection.cursor()
                    isAdmin = isBotAdmin(cursor,update.message.from_user.id)
                    cursor.close()
                    connection.close()
                    if isAdmin:
                        connection = DbConnect()
                                        ...
                                        if update.message.forward_from is not None:
                                            ...
                                            cursor.execute("""
                                                            INSERT INTO Advertise (Advertise_Id,Start_Time,Advertise_Count,Advertise_Period,User_Id,User_Name,User_First_Name,User_Last_Name,User_Full_Name,Active)
                                                                VALUES (%s,%s,1,24,%s,%s,%s,%s,%s,0);",[update.message.message_id,dt,user_id,user_name,user_first_name,user_last_name,user_full_name]
                                                            """)
                                            connection.commit()
                            cursor.close()
                            connection.close()
